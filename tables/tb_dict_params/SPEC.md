...
各参数类型由对应源表目录下的 `insert_param.py` 维护，DS 调度为源表任务的下游。

### 行业分类（`tables/tb_stk_industry/insert_param.py`）
...
### 概念板块（`tables/tb_stk_concept/insert_param.py`）

| c_param_type | 源表前缀 | 说明          |
|--------------|------|-------------|
| 概念板块         | 007  | A股概念板块，日频更新 |
